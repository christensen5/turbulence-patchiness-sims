...
data_v10_B0 = {'nc': filepath_v10_B0, 'V': 10, 'B': 0}
data_v100_B0 = {'nc': filepath_v100_B0, 'V': 100, 'B': 0}
data_v500_B0 = {'nc': filepath_v500_B0, 'V': 500, 'B': 0}
...
for sim in tqdm(all_sims):
    ...
    if sim_id % 3 == 1:
        splots_mag[-1].set_axis_labels("", "B = 0")
        splots_w[-1].set_axis_labels("", "B = 0")
    if sim_id < 4:
        splots_mag[-1].fig.suptitle("V = %d" % sim["V"])
        splots_w[-1].fig.suptitle("V = %d" % sim["V"])
    if sim_id == 7:
        splots_mag[-1].set_axis_labels("|Veff| (mm)", "B = 0")
        splots_w[-1].set_axis_labels("|Veff| (mm)", "B = 0")
    if sim_id > 7:
...
fig.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/Veff/VeffMagvsDepth/VeffMagvsDepth_0B_%dv_semilog_timesnaps.png" % (sim["V"]))
# plt.show()
plt.clf()
...
fig.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/Veff/VeffWvsDepth/VeffWvsDepth_0B_%dv_semilog_timesnaps.png" % (sim["V"]))
# plt.sho