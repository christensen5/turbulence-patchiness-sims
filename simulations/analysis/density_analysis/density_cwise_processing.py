...
timestamps = np.arange(0, 601, 120).tolist() #np.linspace(0, 600, 61, dtype=int).tolist()
...
# load data
lons = np.load(os.path.join(filepath, "lons2.npy"))
lats = np.load(os.path.join(filepath, "lats2.npy"))
deps = np.load(os.path.join(filepath, "deps2.npy"))
...
np.save(os.path.join(filepath, 'density_high2.npy'), density)
