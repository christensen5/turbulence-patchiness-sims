...
if plot_1x3_Bvar:
    ...
    sim_id = 0
    for sim in tqdm([data_v100_B1, data_v100_B3, data_v100_B5]):
        sim_id += 1
        nc = netCDF4.Dataset(sim["nc"])
        deps = nc.variables["z"][:][:, timestamps]
        dir_x = nc.variables["dir_x"][:][:, timestamps]
    ...
    # plt.show()
    fig.savefig(
        "/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/dir/100000p_dir_%s_20-60s_nosurf_100V_Bvar.png" % (
            heatstring))
    # bbox_inches='tight')
