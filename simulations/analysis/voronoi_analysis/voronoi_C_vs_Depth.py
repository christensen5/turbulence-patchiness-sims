...
for simdata_dict in tqdm(all_motile_data):
    ...
    if nosurf:
        fig.suptitle(r'Semi-log Voronoi microbe concentration vs depth (excl. surface microbes)$(B=%3.1fs^{-1}, v=%dums^{-1})$' % (simdata_dict["B"], simdata_dict["V"]), fontsize=18)
    else:
        fig.suptitle(r'Semi-log Voronoi microbe concentration vs depth (incl. surface microbes)$(B=%3.1fs^{-1}, v=%dums^{-1})$' % (simdata_dict["B"], simdata_dict["V"]), fontsize=18)
    fig.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/vor/C/CvsDepth/CvsDepth_vor_%3.1fB_%dv%s_semilog_timesnaps.png" % (simdata_dict["B"], simdata_dict["V"], surfstring))
    # plt.show()
    plt.clf()
...
gs.update(top=0.95)
if nosurf:
    fig.suptitle(r'Semi-log Voronoi microbe concentration vs depth (excl. surface microbes)(non-motile)', fontsize=18)
else:
    fig.suptitle(r'Semi-log Voronoi microbe concentration vs depth (incl. surface microbes)(non-motile)', fontsize=18)
fig.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/vor/C/CvsDepth/CvsDepth_vor_dead%s_semilog_timesnaps.png" % surfstring)