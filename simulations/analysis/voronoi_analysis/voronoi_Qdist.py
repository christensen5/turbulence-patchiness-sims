...
for simdata_dict in tqdm(all_motile_concentrations):
    ...
    if nosurf:
        fig_dists.suptitle(
            r'Distribution of Voronoi-based patch concentration enhancement factor (Q) over time (excl. surface particles) $(f=%3.2f, B=%3.1fs^{-1}, v=%dums^{-1})$' % (
            f, simdata_dict["B"], simdata_dict["V"]), fontsize=18,
        wrap=True)
    else:
        fig_dists.suptitle(
            r'Distribution of Voronoi-based patch concentration enhancement factor (Q) over time (incl. surface particles) $(f=%3.2f, B=%3.1fs^{-1}, v=%dums^{-1})$' % (
                f, simdata_dict["B"], simdata_dict["V"]), fontsize=18,
        wrap=True)
    fig_dists.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/vor/Q/Qdist/Qdist_vor_%3.2ff_%3.1fB_%dv_semilog_timesnaps_%s%s.png" % (f, simdata_dict["B"], simdata_dict["V"], avg_func.__name__, surfstring))
