...
filepath_test = "/home/alexander/Documents/DATA/Ubuntu/Maarten/outputs/results123/initunif/dead/100000p_0-60s_0.01dt_0.1sdt_initunif_dead"
...
# load relevant files
data_dead = {'vols': np.load(os.path.join(filepath_test, 'vols_v.npy')),
             'depth': np.load(os.path.join(filepath_test, 'vols_d.npy')),
             'V': 'dead', 'B': 'dead'}
...
timestamps = np.arange(20, 61, 1)  # timestamps over which to count concentrations. vols_v.npy was computed for 401 timestamps (20-60s in 0.1s steps)
xlims = [-.5, 2500]  #[0, 1.2]
ylims = (1e0, 1e5)
...
for t in tqdm(timestamps):
    ...
    C_dead = C_dead[100 < depths_dead_patches < 170]
