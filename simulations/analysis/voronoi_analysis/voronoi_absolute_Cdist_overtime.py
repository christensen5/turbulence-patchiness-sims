...
timestamps_for_dists = np.arange(0, 61, 12)  # vols_v.npy was computed for 61 timestamps (0-60s in 1s steps)
xlims = [-.5, 1000]  #[0, 1.2]
ylims = (1e0, 1e3)
...
for t in timestamps_for_dists:
    ...
    absC = C_dead  * 5717.87 # convert to microbes per mililitre
    ...
    ax.vlines(avg_func(absC), ylims[0], ylims[1], 'r')
    ax.annotate(" %3.2f" % avg_func(absC), xy=[avg_func(absC), 1e3], xycoords="data", ha="left", va="top", fontsize=15, color="r")
...
if nosurf:
    fig_dists.suptitle(
        r'Distribution of Voronoi-based absolute microbe concentration within patches over time (excl. surface particles) ($f=%3.2f$, non-motile microbes)' % (
        f), fontsize=18,
    wrap=True)
else:
    fig_dists.suptitle(
        r'Distribution of Voronoi-based absolute microbe concentration within patches over time (incl. surface particles) $(f=%3.2f, B=%3.1fs^{-1}, v=%dums^{-1})$' % (
            f), fontsize=18,
    wrap=True)
fig_dists.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/vor/Q/absolute_concentration_overtime/absCdist_vor_%3.2ff_dead_semilog_timesnaps_%s%s.png" % (f, avg_func.__name__, surfstring))
...
for simdata_dict in tqdm(all_motile_concentrations):
    ...
    for t in timestamps_for_dists:
        ...
        ax.vlines(0, ylims[0], ylims[1], 'k')
        ax.vlines(avg_func(absC), ylims[0], ylims[1], 'r')
        ax.annotate(" %3.2f" % avg_func(absC), xy=[avg_func(absC), 1e3], xycoords="data", ha="left", va="top",
                    fontsize=15, color="r")
        # ax.set_xticks([0, 5, 10, 15])
        if int(str(plotindex)[-1]) > 3:
            ax.set_xlabel('Absolute concentration'+r'[$mL^{{-1}}$]', fontsize=15)
    ...
    if nosurf:
        fig_dists.suptitle(
            r'Distribution of Voronoi-based absolute microbe concentration within patches over time (excl. surface particles) $(f=%3.2f, B=%3.1fs^{-1}, v=%dums^{-1})$' % (
            f, simdata_dict["B"], simdata_dict["V"]), fontsize=18,
        wrap=True)
    else:
        fig_dists.suptitle(
            r'Distribution of Voronoi-based absolute microbe concentration within patches over time (incl. surface particles) $(f=%3.2f, B=%3.1fs^{-1}, v=%dums^{-1})$' % (
                f, simdata_dict["B"], simdata_dict["V"]), fontsize=18,
        wrap=True)
    fig_dists.savefig("/media/alexander/DATA/Ubuntu/Maarten/outputs/results123/initunif/comparison/vor/Q/absolute_concentration_overtime/absCdist_vor_%3.2ff_%3.1fB_%dv_semilog_timesnaps_%s%s.png" % (f, simdata_dict["B"], simdata_dict["V"], avg_func.__name__, surfstring))
