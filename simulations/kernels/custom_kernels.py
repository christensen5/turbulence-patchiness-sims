...
def GyrotaxisEE_3D_withTemp(particle, fieldset, time):
    ...
    if not particle.B == 0.0:
        di = 0.5 * ((1 / particle.B * -1 * particle.dir_y * particle.dir_x) + (vort_y * particle.dir_z) - (
                vort_z * particle.dir_y))
        dj = 0.5 * ((1 / particle.B * (1 - particle.dir_y * particle.dir_y)) + (vort_z * particle.dir_x) - (
                vort_x * particle.dir_z))
        dk = 0.5 * ((1 / particle.B * -1 * particle.dir_y * particle.dir_z) + (vort_x * particle.dir_y) - (
...
def GyrotaxisRK4_3D_withTemp(particle,fieldset, time):
    ...
    if not particle.B == 0.0:
        di = 0.5 * (((1/particle.B) * (-1*dir_y1*dir_x1)) + (vort_y1*dir_z1) - (vort_z1*dir_y1))
        dj = 0.5 * (((1/particle.B) * (1-dir_y1*dir_y1)) + (vort_z1*dir_x1) - (vort_x1*dir_z1))
        dk = 0.5 * (((1/particle.B) * (-1*dir_y1*dir_z1)) + (vort_x1*dir_y1) - (vort_y1*dir_x1))
    else:
